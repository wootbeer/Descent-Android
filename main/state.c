...
int state_get_save_file(char * fname, char * dsc, int multi )
{
	...
	if (choice > -1) {
		...
		// here from how NM_TYPE_INPUT_MENU editing feeds keys into this field (see
		// draw_item()'s NM_TYPE_INPUT_MENU case in newmenu.c, and key_to_ascii()'s identity
		// passthrough in io/key.c). A control character doesn't render as anything visible,
		// so a description that's really just an Enter keypress *looks* blank in the Restore
		// Game list later but isn't an empty string, and would slip past a check that only
		// looks for '\0' or spaces.
		for (src = dst = desc[choice]; *src; src++) {
			if (!iscntrl((unsigned char) *src))
				*dst++ = *src;
		}
		*dst = '\0';

		// A blank (or whitespace-only, once the above strip runs) description is confusing
		// to tell apart from the others in the Restore Game list later -- fall back to a
		// plain default name and let the save proceed normally rather than blocking it.
		for (src = desc[choice]; *src; src++) {
			if (*src != ' ') {
				is_blank = 0;
				break;
			}
