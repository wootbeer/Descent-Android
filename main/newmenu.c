...
// Descent/src/main/cpp/keyboard.c. Physical gamepad A, polled and cleared here
// exactly like a touch/tap: newmenu_do3() and newmenu_get_filename() below both
// treat a true return the same as mouse_up on the currently highlighted item,
// so A always means "confirm" on any menu -- even when A is currently bound to
// a gameplay action in the Remap Gamepad menu, since DescentView.java only
// forwards A to that binding while no menu is open.
extern int gamepad_menu_confirm_poll(void);
...
int newmenu_do3( char * title, char * subtitle, int nitems, newmenu_item * item, void (*subfunction)(int nitems,newmenu_item * items, int * last_key, int citem), int citem, char * filename, int width, int height )
{
	...
	int mouse_down = 0;
	int mouse_up = 0;
	int gamepad_confirm = 0;
	// Mirrors what mouse_up already does naturally for a tap: when gamepad_confirm causes a
	// deferred key value (k = KEY_SPACEBAR/KEY_ENTER, below) to be set for NM_TYPE_CHECK,
	// NM_TYPE_RADIO or NM_TYPE_INPUT_MENU, that value is only actually acted on by the
	// switch(k) block further down, which already ran earlier THIS iteration -- so it needs
	// to survive into the very next iteration to have any effect. A tap gets this for free
	// because mouse_up (the same flag that triggered the assignment) is still true at the top
	// of the next iteration, which is specifically what stops k from being clobbered by a
	// fresh key_inkey() poll. gamepad_confirm has no such carry-over on its own, since it's
	// polled-and-cleared every iteration independent of k -- without this flag, a physical A
	// press would set k = KEY_SPACEBAR here, only for it to be overwritten by key_inkey()
	// before the next iteration's switch(k) ever sees it, silently swallowing the toggle. This
	// is why checkboxes could only be flipped by tapping, never by pressing A.
	int gamepad_confirm_pending = 0;
	int slider_locked = 0;
...
#ifdef OGLES
	// Use the game's own stable logical screen size rather than re-querying the live EGL
	// surface (getRenderBufferSize()) -- that query can transiently disagree with the actual
	// GL viewport right around a surface/layout event (keyboard showing, first input after
	// launch), and this size needs to match what ogles_save_screen() itself now uses (see
	// its comment in 2d/gr.c) for the same reason: a mismatch here previously meant this
	// bitmap's declared size and the underlying texture's real captured size could disagree.
	viewWidth = grd_curscreen->sc_w;
	viewHeight = grd_curscreen->sc_h;
#endif
	if ( filename == NULL )	{
#ifdef OGLES
...
// ty = 15 + (yborder/4);
	...
	// Discard any gamepad A press that was still pending from whatever was showing
	// before this menu (e.g. an intro screen the player skipped right as this menu
	// came up) -- key_flush() already does the equivalent for the keyboard buffer
	// at most call sites, but that doesn't reach this separate confirm signal.
	gamepad_menu_confirm_poll();
	...
	while(!done)	{
		...
		if (mouse_up || gamepad_confirm_pending) {
			mouse_down = 0;
			gamepad_confirm_pending = 0;
		} else {
			k = key_inkey();
		}
		...
		// Physical gamepad A -- treated as a confirm on the currently highlighted
		// item, same as a tap (see the mouse_up handling below).
		gamepad_confirm = gamepad_menu_confirm_poll();

		// Get the "mouse" (AKA, touch screen)
		mouse_up = mouse_button_up_count(0, &mouse_x, &mouse_y);

		// render.c's showRenderBuffer() injects a synthetic mouse_handler(-1,-1,true) +
		// mouse_handler(-1,-1,false) pair after the EGL surface/context gets torn down and
		// recreated (Surface_was_destroyed), purely to force whatever menu is on screen to
		// notice something changed and redraw -- it's not a real tap and (-1,-1) isn't a real
		// position. Treated as ordinary touch data, it looked exactly like a tap landing
		// outside this menu's box (-1 is always less than the menu's x/y), which hit the
		// "tapped outside, cancel" branch below and silently closed the menu with no
		// selection made -- or, if it arrived while a real finger-down was already being
        // tracked, it clobbered that in-progress gesture instead. Since the main menu screen
		// is exactly where a tap is already known to be able to trigger a surface/system-UI
		// churn (see the immersive-mode notes elsewhere in this codebase), this could race
		// with the very tap trying to make a selection -- visible as the highlighted item
		// (e.g. "Change Pilots...") flashing back to its unselected color and nothing else
		// happening. Consume it here as a "just redraw everything" cue instead of letting it
		...
		if (mouse_x == -1 && mouse_y == -1) {
			...
			if (mouse_up && (mouse_x < x || mouse_y < y || mouse_x > x + w || mouse_y > y + h)) {
				...
				// choice can legitimately be -1 here (touch moved off every item, but still
				// inside the menu's outer box) -- item[-1] is out of bounds, so only mark the
				// newly-highlighted item dirty when there actually is one.
...
//			done=1;
		...
		if ((mouse_up || gamepad_confirm) && choice != -1) {
			switch (item[choice].type) {
				case NM_TYPE_INPUT_MENU:
					k = KEY_ENTER;
					// See gamepad_confirm_pending's declaration above -- without this,
					// pressing A on this item type would silently do nothing.
					if (gamepad_confirm) gamepad_confirm_pending = 1;
					break;
				case NM_TYPE_CHECK:
				case NM_TYPE_RADIO:
					k = KEY_SPACEBAR;
...
int newmenu_get_filename( char * title, char * filespec, char * filename, int allow_abort_flag )
{
	...
	// Discard any gamepad A press still pending from whatever was showing before
	// this screen -- this is the startup pilot-select screen's own entry point,
	// so without this, an A pressed during the intro screens beforehand would
	// otherwise queue up and immediately confirm whatever pilot happens to be
	// highlighted the instant this screen appears.
	gamepad_menu_confirm_poll();
...
ReadFileNames:
	...
	while(!done)	{
		...
		// Physical gamepad A -- treated as a confirm on the currently highlighted
		// file/pilot, same as a tap (see the mouse_up handling below). This is what
		// lets A pick a pilot on the startup pilot-select screen, which otherwise
		// has no gamepad confirm at all.
		gamepad_confirm = gamepad_menu_confirm_poll();
		...
		if (!mouse_down) {
			...
			// Only move the highlight if the finger is actually resting over a real row --
			// this used to unconditionally overwrite citem every frame the finger stayed
			// down, including with -1 ("no row here"), which the clamp below then pinned to
			// item 0. A touch left over from tapping through the intro/cutscene screens can
			// still read as "held" the instant this screen appears (there's no frame
			// boundary between the two), and with the finger resting somewhere that isn't a
			// pilot row, that pinned the highlight in place, fighting the D-pad/stick
			// navigation below on every single iteration until the finger was fully lifted.
			{
				int hit = get_filename_item_at_pos(mouse_x, mouse_y, NumFiles, items);
				if (hit != -1) {
					citem = hit;
				}
			}
		}
		if (mouse_up || gamepad_confirm) {
			mouse_down = 0;
			if (citem != -1) {
				done = true;
			}
		...
		// The KEY_UP/KEY_PAD8 case above only ever decrements citem -- with no
		// lower clamp, repeatedly scrolling up past the first entry (e.g. "Create
		// new pilot") sent citem negative. That doesn't match any real row, so the
		// highlight-drawing check below (i>=0 && i<NumFiles) silently stopped
		// drawing any highlight at all, while citem kept sinking further and
		// further out of range with every extra press.
		if (citem < 0)
			citem = 0;
