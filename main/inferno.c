...
// exactly like a touch/tap -- see show_order_form()'s skip check below.
...
void show_order_form()
{
	...
	if ((pcx_error = pcx_read_bitmap(exit_screen, &bmp, BM_LINEAR, title_pal)) == PCX_ERROR_NONE) {
		...
		free(bmp.bm_data);
		// Fewer steps than the game's usual 32 -- gr_sync_display() (2d/modexc.c)
		// sleeps ~17ms per step to loosely pace the fade, so 32 steps adds up to
		// over half a second of sleep on top of however long the actual drawing
		// takes. That's fine for in-game fades, but on this outro/warning screen it
		// reads as a sluggish delay before the skip visibly registers. Only this
		// screen's fades are shortened; gr_sync_display() itself and every other
		// fade in the game (menus, level end, death, etc.) are untouched.
		gr_palette_fade_in(title_pal, 12, 0);
		skipped_by_user = 0;
		{
			int done = 0;
			...
			while (!done)	{
				...
				if (mouse_button_down_count(0, &x, &y)) { done = 1; skipped_by_user = 1; }
				// Physical gamepad A skips this screen exactly like a tap does -- the
				// outro screen shown after quitting from the main menu.
				if (gamepad_menu_confirm_poll()) { done = 1; skipped_by_user = 1; }
			}
		}
		// This app has no graceful Android shutdown path -- inferno_main() calls the C
		// library's exit() directly right after this function returns (see its call site),
		// which kills the whole process immediately rather than going through Activity.finish()
		// and the normal window teardown. When the player actively skips (tap/A), that abrupt
		// exit() follows almost immediately -- there's no reliable way from here to guarantee
		// Android's compositor has actually presented a given frame before the process dies, so
		// fading to black first just adds a real chance the app closes on whatever frame it last
		// managed to display instead of the final black one, which looks like this screen
		// flashing back at full brightness right as the app quits. Skipping the fade entirely on
		// an active skip sidesteps that race completely -- there's no black frame to race against
		// exit() for -- and the app just closing outright is exactly what the player asked for by
		// skipping. The timeout path (nobody's actively watching at that point) still fades
		// normally, matching every other exit/warning screen fade in the game.
		if (!skipped_by_user) {
			// Shortened to match the fade-in above -- see that comment for why.
			gr_palette_fade_out(title_pal, 12, 0);
		}
	}
	key_flush();
}
...
//	}
   ...
   gr_use_palette_table( "PALETTE.256" );
	...
	// "-quickresume" is passed in by DescentView/main.c only for the one launch immediately
	// following a settings-triggered auto-restart (see restartAppForSettingsChange() in
	// main/menu.c and DescentActivity.java) -- skip the intro logos entirely on that launch so
	// re-applying a video setting feels close to instant instead of replaying the whole boot.
	if ( !FindArg( "-quickresume" ) )
	{
	#ifndef RELEASE
	if ( !FindArg( "-notitles" ) )
	#endif
	...
	init_game();
	// The "Detail Levels" preset menu is gone now -- Video Options' individual sliders are
	// the only way to change detail settings. Apply whatever detail level was last in effect
	// (a preset from before this change, or already-custom) once, then permanently switch to
	// the custom sentinel so future adjustments (and config saves) always go through the
	// individual settings instead of a preset. See set_detail_level_parameters() and
	// config.c's detail_level_str handling for how the custom values are persisted.
	...
	// Seamless-resume, part 3 of 3 (see the "-quickresume" intro-logo skip above and
	// RegisterPlayer() in main/gameseq.c for the other two): with the intro logos and pilot
	// picker already skipped, the very next thing the loop below does is show the normal main
	// menu (FMODE_MENU -> DoMenu()) -- which is exactly what we want here, since it comes with
	// its usual background art already in place. (An earlier version of this landed straight
	// in the Options menu instead, but that submenu has no background of its own to show, so
	// it looked broken sitting on top of nothing.)
