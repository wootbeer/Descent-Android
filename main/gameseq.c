...
RegisterPlayer()
{
	...
	// Seamless-resume, part 2 of 3 (see the intro-logo skip and do_options_menu() call in
	// main/inferno.c for the other two): on the one launch immediately following a
	// settings-triggered auto-restart, skip the pilot picker entirely and load the last-used
	// pilot directly -- Players[Player_num].callsign was already set to config_last_player
	// just above. Falls through to the normal picker below if there's somehow no last pilot on
	// file (can't really happen in practice, since this restart is only ever triggered from
	// inside the Options menu, which requires an active pilot already).
	if ( FindArg( "-quickresume" ) && Players[Player_num].callsign[0] != 0 )	{
		read_player_file();
