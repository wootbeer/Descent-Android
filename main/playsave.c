...
// Gamepad button remap table -- Descent/src/main/cpp/gamepad_remap.c. Persisted here
...
// see Descent/src/main/cpp/gamepad_remap.c. Persisted as one more trailing byte, same
// pattern as everything above.
extern int Gamepad_stick_layout;
...
RetrySelection:
	...
	Auto_leveling_on = Default_leveling_on = 0;
	n_highest_levels = 1;
...
int read_player_file()
{
	...
	//read kconfig data
	{
		...
		// trailing fread below -- this process can load more than one pilot (the
		// pilot picker can switch mid-session), so a pilot with no saved remap data
		// (new, or a .plr predating this feature) must not silently inherit
		// whatever a previously-loaded pilot left in the live table.
		gamepad_remap_reset_to_defaults();

		// Same reasoning as above, for Invert Y: seed the compiled-in default (on)
		// before the trailing read below, so a pilot with no saved value doesn't
		// silently inherit whatever a previously-loaded pilot left it as.
		Config_invert_y = 1;

		// Same reasoning again, for Use Gyroscope: seed the compiled-in default
		// (off) before the trailing read below, so a pilot with no saved value
		// (new, or a .plr predating this field) doesn't silently inherit whatever
		...
		if (fread( kconfig_settings, MAX_CONTROLS*CONTROL_MAX_TYPES, 1, file )!=1)
			...
			errno_ret=errno;
		else if (fread(gamepad_keycodes, sizeof(short), GAMEPAD_REMAP_NUM_ACTIONS, file) != GAMEPAD_REMAP_NUM_ACTIONS)
			errno_ret=errno;
		else {
			// Bindings read successfully -- commit them now, independent of whether the
			// stick-layout byte below (added after this array, in a later revision) is
			// actually present. Gating this commit on that later read too (as a single
			// else-if chain would) meant a save written after gamepad remapping existed
			// but before stick-layout did would fail the trailing fread below and silently
			// discard perfectly good, already-read button bindings back to defaults --
			// so stick-layout gets its own independent, failure-tolerant read instead.
			for (gi = 0; gi < GAMEPAD_REMAP_NUM_ACTIONS; gi++)
				Gamepad_bound_keycodes[gi] = gamepad_keycodes[gi];

			// A save from before this field existed simply fails this fread and leaves
			// Gamepad_stick_layout at the default gamepad_remap_reset_to_defaults()
			// already seeded above -- same graceful-degrade behavior as every other
			// trailing field in this file.
			if (fread(&stick_layout, sizeof(ubyte), 1, file) == 1)
				Gamepad_stick_layout = stick_layout;

			// Same independent, failure-tolerant pattern for Invert Y -- a save from
...
//	}
	...
	//write kconfig info
	{
		...
		if (fwrite( kconfig_settings, MAX_CONTROLS*CONTROL_MAX_TYPES, 1, file )!=1)
			...
			errno_ret=errno;
		else if (fwrite( gamepad_keycodes, sizeof(short), GAMEPAD_REMAP_NUM_ACTIONS, file ) != GAMEPAD_REMAP_NUM_ACTIONS)
			errno_ret=errno;
