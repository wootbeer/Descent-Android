...
// The FPS counter used to be developer-only (compiled out entirely in RELEASE builds).
// It's now a normal user-facing option -- see the "Show FPS Counter" checkbox in
// Options -> Detail Level Customization (main/menu.c) -- so it's no longer gated by
// RELEASE/NDEBUG; framerate_on (declared above) controls it in every build.
