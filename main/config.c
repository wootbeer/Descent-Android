...
static char *render_scale_str = "RenderScale";
static char *aspect_ratio_43_str = "AspectRatio43";
static char *framerate_on_str = "ShowFPS";
...
extern int Render_scale_index;
extern int Aspect_ratio_4_3;
...
int ReadConfigFile()
{
	...
	while (!feof(infile)) {
		...
		if (*ptr != '\0') {
			...
			else if (!strcmp(token, render_scale_str)) {
				Render_scale_index = strtol(value, NULL, 10);
				if (Render_scale_index < 0 || Render_scale_index > 4)
					Render_scale_index = 0;
			}
			else if (!strcmp(token, aspect_ratio_43_str)) {
				Aspect_ratio_4_3 = strtol(value, NULL, 10) ? 1 : 0;
			}
			else if (!strcmp(token, framerate_on_str)) {
				framerate_on = strtol(value, NULL, 10) ? 1 : 0;
			}
...
int WriteConfigFile()
{
	...
	if (Detail_level == NUM_DETAIL_LEVELS-1)
		...
		sprintf (str, "%s=%d\n", detail_level_str, Detail_level);
	fputs(str, infile);
	sprintf (str, "%s=%d\n", render_scale_str, Render_scale_index);
	fputs(str, infile);
	sprintf (str, "%s=%d\n", aspect_ratio_43_str, Aspect_ratio_4_3);
	fputs(str, infile);
	sprintf (str, "%s=%d\n", framerate_on_str, framerate_on);
	fputs(str, infile);
