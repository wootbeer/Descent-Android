...
// exactly like a touch/tap -- see show_title_screen()'s skip check below.
...
int local_key_inkey(void)
{
	...
	// Physical gamepad A -- same "confirm/skip" meaning as a tap (see the touch check
	// above). This is what makes A advance/skip briefing text as it's being typed out
	// (show_briefing_message() below drives all of that off local_key_inkey(), same as
	// the "wait for a key to continue" pauses between pages), on top of the intro-title-
	// screen skip show_title_screen() already does with the same poll.
	if ( gamepad_menu_confirm_poll() ) {
		rval = KEY_ENTER;
	}
...
int show_title_screen( char * filename, int allow_keys )	
{
	...
	// Fewer steps than the game's usual 32 -- gr_sync_display() (2d/modexc.c) sleeps
	// ~17ms per step to loosely pace the fade, so 32 steps adds up to over half a
	// second of sleep on top of however long the actual drawing takes. That's fine
	// for in-game fades, but on this splash screen it reads as a sluggish delay
	// before the skip visibly registers. Only these intro-logo fades are shortened;
	// gr_sync_display() itself and every other fade in the game (menus, level end,
	// death, etc.) are untouched.
	gr_palette_fade_in(New_pal, 12, allow_keys);

	gr_palette_load( New_pal );
	timer	= timer_get_fixed_seconds() + i2f(3);
	
	while (1)	{
		// Physical gamepad A skips this screen exactly like a tap does. allow_keys
		// is checked first (short-circuiting) so a screen that doesn't accept skip
		// input at all doesn't consume a pending A press meant for a later one.
		if ( allow_keys && (mouse_button_down_count(0, NULL, NULL) > 0 || gamepad_menu_confirm_poll()) ) break;
	...
	gr_palette_fade_out(New_pal, 12, allow_keys);
...
int show_briefing_message(int screen_num, char *message)
{
	...
	// Scaled locally rather than mutating bsp->text_ulx/text_uly in place: bsp points
	// straight into the persistent, file-scope Briefing_screens[] array, and this function
	// runs again for the same screens every time briefing screens are shown (i.e. every
	// "New Game", not just the app's first launch). Multiplying bsp's own fields in place
	// meant every replay re-scaled an already-scaled value, so text_ulx/text_uly grew by
	// another factor of Scale_x/Scale_y on each pass -- fine (and invisible) the first time,
	// but by the second briefing the text position had been pushed off-canvas entirely: the
	// bitmap (positioned independently of this) still showed, but the text never did.
	int text_ulx = bsp->text_ulx * f2fl(Scale_x);
	int text_uly = bsp->text_uly * f2fl(Scale_y);
...
//				prev_ch = 10;							//	read to eoln
			...
			if (prev_ch != '\\') {
				...
				if (Briefing_text_y > text_uly + f2fl(Scale_y) * bsp->text_height) {
		...
		if (Briefing_text_x > text_ulx + f2fl(Scale_x) * bsp->text_width) {
			Briefing_text_x = text_ulx;
			Briefing_text_y += 8 * f2fl(Scale_y);
		}

		if ((new_page) || (Briefing_text_y > text_uly + f2fl(Scale_y) * bsp->text_height)) {
			fix	start_time = 0;
