...
GLuint ogles_save_screen() {
	...
	// Use the game's own stable logical screen size (set once at startup, see gr_init()
	// above) rather than re-querying the live EGL surface via getRenderBufferSize(). That
	// query (eglQuerySurface) can transiently disagree with the GL viewport actually in use
	// -- e.g. right around a surface/layout event like the on-screen keyboard showing, or
	// the very first input after launch reasserting immersive mode -- and grd_curscreen's
	// values are exactly what every other 2D draw call (scale_bitmap_ogles(), gr_rect_ogles())
	// already treats as authoritative for this same screen. If this had ever asked
	// glCopyTexImage2D to copy a region even slightly larger than what was actually
	// rendered into the framebuffer, the extra rows/columns it copied would have been
	// whatever leftover/uninitialized memory happened to be there, baked permanently into
	// this saved-screen texture -- and since this texture then gets stretched back across
	// the whole display by ogles_draw_saved_screen(), that garbage would show up as a thin
	// stray-colored line right at the screen's edge, exactly where the missing coverage was.
	glEnable(GL_TEXTURE_2D);
	glGenTextures(1, &tex);
	...
	glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, grd_curscreen->sc_w, grd_curscreen->sc_h, 0);
	return tex;
}
