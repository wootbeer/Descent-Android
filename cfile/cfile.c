...
#ifdef ANDROID_NDK
	fp = NULL;
	// Prefer a real file in the app's document directory first -- this is where
	// user-supplied game data (DESCENT.HOG / DESCENT.PIG, picked via Storage Access
	// Framework and copied in by DescentActivity) lives. Fall back to the bundled
	// APK assets (if any) so nothing else breaks.
	if (Document_path) {
